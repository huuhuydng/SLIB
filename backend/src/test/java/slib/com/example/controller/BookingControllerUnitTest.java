...
class BookingControllerUnitTest {
    ...
    void createBooking_validData_returns200WithReservation() throws Exception {
        ...
        when(bookingService.createBooking(eq(userId), eq(seatId), any(LocalDateTime.class), any(LocalDateTime.class)))
        ...
        verify(bookingService, times(1)).createBooking(eq(userId), eq(seatId), any(LocalDateTime.class), any(LocalDateTime.class));
    }
    ...
    void getBookingsByUser_validUserId_returns200WithReservationsList() throws Exception {
        // Arrange
        UUID userId = UUID.randomUUID();
        ReservationEntity res1 = createReservationEntity(UUID.randomUUID(), userId, 1, "CONFIRMED", 
                                                         LocalDateTime.now(), LocalDateTime.now().plusHours(2));
        ReservationEntity res2 = createReservationEntity(UUID.randomUUID(), userId, 2, "CHECKED_IN", 
                                                         LocalDateTime.now().plusDays(1), LocalDateTime.now().plusDays(1).plusHours(1));
        List<ReservationEntity> reservations = Arrays.asList(res1, res2);

        when(bookingService.getBookingsByUser(userId)).thenReturn(reservations);
        ...
        verify(bookingService, times(1)).getBookingsByUser(userId);
    }
    ...
    void getBookingsByUser_noBookings_returns200WithEmptyArray() throws Exception {
        // Arrange
        UUID userId = UUID.randomUUID();
        when(bookingService.getBookingsByUser(userId)).thenReturn(List.of());
