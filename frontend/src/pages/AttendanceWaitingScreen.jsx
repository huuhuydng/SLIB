  ...
  useEffect(() => {
    ...
    const fetchInitialLogs = async () => {
      try {
        ...
        const historyLogs = Array.isArray(data) ? data.slice(0, 7).map((item, index) => ({
    ...
    const onConnected = () => {
      stompClient.subscribe('/topic/access-logs', (payload) => {
        ...
        setTimeout(() => { if (isMounted) setLatestActivity(null); }, 1500);
        
        // Luôn giữ tối đa 5 bản ghi mới nhất
        setLogs(prevLogs => [newEntry, ...prevLogs].slice(0, 7)); 
      });
    };
