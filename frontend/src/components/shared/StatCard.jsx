 ...
 * @param {string} bg - Background color for icon container
 * @param {string} color - Icon color
 * @param {string} trend - "up" | "down" | "neutral" (optional)
 * @param {string} trendValue - Trend percentage or value (optional)
 * @param {string} variant - "default" | "primary" | "success" | "warning" | "error" (optional)
 */
const StatCard = ({ 
  icon, 
  value, 
  ...
  const getTrendColor = () => {
    switch (trend) {
      case 'up':
        return 'var(--slib-status-success, #388E3C)';
      case 'down':
        return 'var(--slib-status-error, #D32F2F)';
      default:
        return 'var(--slib-text-muted, #A0AEC0)';
    }
  };
