    ...
    public class AreaFactoryService {
        ...
        public AreaFactoryResponse getAreaFactoryById(Long factoryId) {
            return areaFactoryRepository.findById(factoryId)
                    .map(this::toResponse)
                    .orElseThrow(() ->
                            new RuntimeException("Area Factory không tồn tại với ID: " + factoryId));
        }
        ...
        public AreaFactoryResponse createAreaFactory(Long areaId, AreaFactoryEntity entity) {
            AreaEntity area = areaRepository.findById(areaId)
                    .orElseThrow(() ->
                            new RuntimeException("Area không tồn tại với ID: " + areaId));
        ...
        public AreaFactoryResponse updateAreaFactory(Long factoryId, AreaFactoryEntity req) {
            ...
            if (req.getFactoryName() != null) factory.setFactoryName(req.getFactoryName());
            if (req.getPositionX() != null) factory.setPositionX(req.getPositionX());
            if (req.getPositionY() != null) factory.setPositionY(req.getPositionY());
            if (req.getWidth() != null) factory.setWidth(req.getWidth());
            if (req.getHeight() != null) factory.setHeight(req.getHeight());
            if (req.getColor() != null) factory.setColor(req.getColor());
        ...
        public AreaFactoryResponse resizeAreaFactory(Long factoryId, Integer width, Integer height) {
            ...
            if (width != null && width > 0) factory.setWidth(width);
            if (height != null && height > 0) factory.setHeight(height);
