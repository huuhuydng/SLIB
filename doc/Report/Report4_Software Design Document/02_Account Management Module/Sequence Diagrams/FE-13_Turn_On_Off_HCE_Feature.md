...
sequenceDiagram
    ...
    Users->>Client: 1. Toggle the HCE feature setting
    activate Users
    activate Client

    alt 2a. User turns HCE on
        Client->>Client: 2a.1 Set isHceEnabled = true in UI state
    else 2b. User turns HCE off
        Client->>Client: 2b.1 Set isHceEnabled = false in UI state
    end
    ...
    Client->>UserSettingController: 5. PUT /slib/settings/{userId} with isHceEnabled
    deactivate Client
    ...
    UserSettingService->>UserSettingService: 10. Apply new HCE preference
    ...
    alt 13a. Server synchronization succeeds
        ...
        deactivate UserSettingController
        activate Client
        Client-->>Users: 13a.2 Keep HCE switch in the new state
